...
-- Update existing members without TNS numbers
UPDATE public.membership_registrations 
SET tns_number = (
    SELECT 'TNS' || LPAD(
        (ROW_NUMBER() OVER (ORDER BY created_at) + 
         COALESCE(
             (SELECT MAX(CAST(SUBSTRING(tns_number FROM 4) AS INTEGER)) 
              FROM public.membership_registrations 
              WHERE tns_number ~ '^TNS[0-9]+$'), 0
         ))::TEXT, 3, '0'
    )
    FROM (
    ...
    WHERE ranked.id = membership_registrations.id
)
WHERE tns_number IS NULL;