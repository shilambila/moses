...
serve(async (req) => {
  ...
  try {
    const mysql = createClient(mysqlUrl, mysqlServiceKey)
    ...
    if (req.method === 'POST') {
      ...
      if (json && typeof json === 'object' && 'action' in json) {
        ...
        if (action === 'stk_push') {
          return await handleSTKPush(data, mysql);
        }
        if (action === 'callback') {
          return await handleSTKCallback(data, mysql);
        }
        return new Response(
      ...
      if (json && json.Body && json.Body.stkCallback) {
        return await handleSTKCallback(json, mysql);
      }
...
async function handleSTKPush(data: any, mysql: any) {
  ...
  try {
    ...
    if (stkResult.ResponseCode === '0') {
      ...
      const { error: dbError } = await mysql
        .from('mpesa_payments')
        .insert({
          member_id: memberId,
          amount: parseFloat(amount),
...
async function handleSTKCallback(callbackData: any, mysql: any) {
  try {
    ...
    if (resultCode === 0) {
      const { data: paymentData } = await mysql
        .from('mpesa_payments')
      ...
      if (paymentData) {
        ...
        const { error: contributionError } = await mysql
          .from('contributions')
          .insert({
